...
const PageScrollIconsMenu: React.FC = () => {
  ...
  const sectionIdArray = pageLinkData.map((link) => link.url);
  ...
  return (
    <div className="fixed md:right-[5%] xl:right-[5%] top-[50%] -translate-y-[25%] z-[1000] hidden md:block">
      <ul className="flex flex-col sm:flex-col gap-[2rem]">
        {links.map((link) => {
          const { id, url, icon, tooltip_content } = link;
          return (
            <Tooltip
            ...
            >
              ...
              >
                <button
                  onClick={() => {
                    const scrollToElement = document.getElementById(url);
                    if (scrollToElement) {
